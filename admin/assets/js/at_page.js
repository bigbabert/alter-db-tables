jQuery(document).ready(function($) {
	...
	function blur_file_upload_field() {
		...
		// Setup ajax variable
		var data = {
			action: 'at_csv_to_db_get_csv_cols',
	...
	$('#table_select').change(function() {  // Get column count and load table
		...
		$('#table_preview').html('<img src="'+at_csv_to_db_pass_js_vars.ajax_image+'" />');
		...
		// Setup ajax variable
		var data = {
			action: 'at_csv_to_db_get_columns',
			sel_val: sel_val
			//disable_autoinc: disable_autoinc
		};
		
		// Run ajax request
		$.post(at_csv_to_db_pass_js_vars.ajaxurl, data, function(response) {
